...
> 以下步骤假设你已安装好 Python 3.11+ 和 Neo4j 5.x，并且 Neo4j 服务正在运行。

```bash
# 1. 克隆项目
git clone <repo-url> LiCunXu
cd LiCunXu
...
# 4. 初始化图谱（加载种子数据 + 从已有抽取结果恢复，约 5 分钟）
...
打开浏览器访问 **http://localhost:8000**，即可看到知识图谱可视化界面。

- 🔍 在搜索框输入人名（如「李存勖」），回车查看人物关系图谱
- 💬 切换到「智能问答」标签页，用自然语言提问（如「李克用有哪些义子？」）
- ✏️ 点击任意人物节点 → 右侧详情面板中可直接编辑关系、别名、删除错误节点

> 如果你需要从零开始用 LLM 抽取知识（全量摄入），请参阅下方[三、快速启动 → 数据准备与摄入](#数据准备与摄入)。
...
- **在线编辑**：在人物/事件详情面板中可直接修改图谱数据（修改关系类型、新增/删除关系、编辑别名、删除错误节点），所有操作即时写入 Neo4j
...
支持直接在 Web 界面上修正图谱中的错误，无需重新摄入数据：

- **修改节点间关系**：在人物详情面板中，每条关系旁有「改」按钮，可将关系类型从 `SUBORDINATE`（臣属）改为 `FATHER_OF`（父子）等
- **新增节点间关系**：点击「＋新增关系」按钮，指定目标人物、关系类型和方向，支持一人多关系（如既是兄弟也是君臣）
- **删除节点间关系**：每条关系旁有「删」按钮，二次确认后删除
- **删除错误节点**：人物/事件详情面板底部有「🗑️ 删除此人物/事件」按钮，会同时删除该节点的所有关系
- **修改人物别名**：别名行旁有「编辑别名」按钮，支持逐个添加/删除别名（解决官职被误识别为别名的问题）

所有编辑操作即时生效，通过 RESTful API 直接写入 Neo4j。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| DELETE | `/api/graph/node/{uid}` | 删除节点及其所有关系 |
| POST | `/api/graph/relation/delete` | 删除两个节点之间的指定关系 |
| POST | `/api/graph/relation/add` | 在两个已有节点之间新增关系 |
| POST | `/api/graph/relation/update` | 修改关系类型（先删旧、再建新） |
| POST | `/api/graph/person/aliases` | 更新人物别名列表 |
| GET | `/api/graph/relation_types` | 获取图谱中所有已使用的关系类型 |
...
│   ├── crud.py             # 图谱 CRUD（智能合并 + 在线编辑：删除节点/关系、修改关系、更新别名）
...
│
├── api/                    # Web API 层
│   ├── routes.py           # FastAPI 路由（图谱查询 + 人物/事件详情 + 图谱编辑 + 问答 + 管理）
│   └── visualization.py    # 前端 HTML 文件读取
│
├── static/                 # 前端静态资源
│   └── index.html          # 单页应用（vis-network 图谱可视化 + 问答面板 + 详情面板 + 在线编辑）
│
├── scripts/                # 工具脚本
